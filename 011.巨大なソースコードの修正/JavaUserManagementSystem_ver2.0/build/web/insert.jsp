...
<html>
    ...
    <form action="insertconfirm" method="POST">
        ...
        <select name="year">
            ...
            <option value="<%=i%>" <% if(reinput && udb.getYear() == i){ %>selected<% } %>><%=i%></option>
            <% } %>
        </select>年
        <select name="month">
            <option value="">--</option>
            <% for(int i = 1; i<=12; i++){ %>
            <option value="<%=i%>" <% if(reinput && udb.getMonth() == i){ %>selected<% } %>><%=i%></option>
            <% } %>
        </select>月
        <select name="day">
            <option value="">--</option>
            <% for(int i = 1; i<=31; i++){ %>
            <option value="<%=i%>"<% if(reinput && udb.getDay() == i){ %>selected<% } %>><%=i%></option>
            <% } %>
        </select>日
        <br><br>
        ...
        <br>
            <% for(int i = 1; i<=3; i++){ %>
            <label><input type="radio" name="type" value="<%=i%>"<%if(reinput && udb.getType() == i){ %>checked<% } %>><%=jh.exTypenum(i)%></label><br>
            <% } %>
        <br>
